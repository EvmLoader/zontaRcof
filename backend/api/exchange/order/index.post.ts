...
export default async (data: Handler) => {
  ...
  try {
    ...
    if (side === "SELL" && amount < minAmount) {
      throw new Error(
        `Amount is too low. Minimum is ${minAmount.toFixed(
          amountPrecision
        )} ${currency}`
      );
    }
    ...
    if (side === "BUY" && cost < minCost) {
      throw new Error(
        `Cost is too low. Minimum is ${minCost.toFixed(pricePrecision)} ${pair}`
      );
    }
    ...
    await sequelize.transaction(async (transaction) => {
      if (side === "BUY") {
        ...
        if (orderData.status === "closed" || orderData.status === "filled") {
          const netAmount = Number(orderData.amount) - orderData.fee;
          await updateWalletQuery(
            currencyWallet.id,
