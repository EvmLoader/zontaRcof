...
const StakingPoolEdit: React.FC = () => {
  ...
  const fetchPoolData = async () => {
    if (!id) return;
    const { data, error } = await $fetch({
      url: `/api/admin/ext/staking/pool/${id}`,
      silent: true,
    });
    if (!error && data) {
      setName(data.name);
      ...
      setType(
        ...
        ].find((type) => type.value === data.type) || null
      );
      setCurrency(
        formData?.currencies[data.type].find(
          (cur) => cur.value === data.currency
        ) || null
      );
      setChain(
        formData?.chains[data.currency]?.find((chain) => chain === data.chain)
          ? { value: data.chain, label: data.chain }
          : null
      );
      setMinStake(data.minStake.toString());
      setMaxStake(data.maxStake.toString());
      setStatus(
        [
          { value: "ACTIVE", label: t("Active") },
          { value: "INACTIVE", label: t("Inactive") },
          { value: "COMPLETED", label: t("Completed") },
        ].find((status) => status.value === data.status) || null
      );
      setImageUrl(data.icon); // Set the existing image URL
    }
  };
  ...
  const handleFileUpload = async (files: File[]) => {
    if (files.length > 0) {
      ...
      const result = await imageUploader({
        ...
        oldPath: imageUrl || undefined, // Pass the old path if available for replacement
      });
  ...
  return (
    <Layout title={t("Edit Staking Pool")} color="muted">
      <Card className="p-5 mb-5 text-muted-800 dark:text-muted-100">
        <div className="flex flex-col md:flex-row justify-between items-center">
          <h1 className="text-lg mb-4 md:mb-0">{t("Edit Staking Pool")}</h1>
...
export default StakingPoolEdit;
