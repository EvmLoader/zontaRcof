...
export const useEcommerceStore = create<EcommerceStore>()(
  immer((set, get) => ({
    ...
    setCategory: async (categoryName: string) => {
      ...
      const decodedCategoryName = decodeURIComponent(categoryName); // Decode the category name here
    ...
    fetchProduct: async (name: string) => {
      const decodedName = decodeURIComponent(name); // Decode the product name
      const { data, error } = await $fetch({
        url: `/api/ext/ecommerce/product/${decodedName.replace(/-/g, " ")}`,
        silent: true,
      });
